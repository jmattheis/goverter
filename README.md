...
### Struct field name folding

With `goverter:fold` tag you can instruct goverter to perform case-insensitive mapping between
source and target fields. This is useful when converting between two structs auto-generated by
two different tools, each following different name casing rules for struct fields: goverter
will match the fields using strings.EqualFold method if fold tag is present.
...
If fold is present and goverter detects ambiquous match, it will either prefer an exact match if
such is found or report an error. Use either goverter:map to make an explicit mapping choice
or goverter:ignore to ignore irrelevant ambiguous fields.

`goverter:fold` takes no parameters.
...
type Output struct {
    UUID string // auto-matched with Uuid due to goverter:fold
