...
In Go a function has _params_ and _results_. E.g.
...
1. `target` is **first** **result** which `sources` are converted to.
...
interface is grouped into `context` regardless of the param name.
...
func ConvertFour(ctxOne A, source B ctxTwo C) (D, error) {/**/}
// A=context; B=source; C=context; D=target; error=target_error
```
